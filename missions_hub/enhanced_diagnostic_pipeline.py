...
class MedicalEmbedder:
    """
    Lớp 2: Embedding với model y tế.

    Backend priority:
      1. sentence-transformers (BioBERT / MiniLM) — if importable
      2. Ollama REST API (mxbai-embed-large) — always available fallback

    Cách đổi model:
      embedder = MedicalEmbedder(model_name="biobert")  # sentence-transformers
      embedder = MedicalEmbedder(model_name="ollama")   # explicit Ollama
    """
    ...
    def __init__(
        ...
        # Try sentence-transformers first; gracefully fall back to Ollama
        resolved = self.RECOMMENDED_MODELS.get(model_name, model_name)
        if model_name != "ollama":
            try:
                import os as _os
                # Suppress torchao warnings that crash the import
                _os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")
                from sentence_transformers import SentenceTransformer  # type: ignore
                self._st_model = SentenceTransformer(resolved)
                self.model_name = resolved
                self._dim = self._st_model.get_sentence_embedding_dimension()
                logger.info(f"sentence-transformers loaded: {resolved} (dim={self._dim})")
                return
            except Exception as _st_err:
                logger.warning(
                    f"sentence-transformers unavailable ({_st_err.__class__.__name__}: {_st_err}). "
                    "Falling back to Ollama embeddings."
                )

        # Ollama backend
        self.model_name = f"ollama:{ollama_model}"
        self._dim = self.OLLAMA_DIM
        logger.info(f"MedicalEmbedder using Ollama backend ({ollama_model})")

    # ── Internal helpers ───────────────────────────────────────────────────

    def _embed_ollama(self, text: str) -> np.ndarray:
        """Single text embed via Ollama REST."""
    ...
    def embed_cached(self, text: str) -> tuple:
        """Cache embedding — avoids re-compute for identical queries."""
    ...
    def embed(self, text: str) -> np.ndarray:
        return np.array(self.embed_cached(text), dtype=np.float32)

    def embed_batch(self, texts: list[str]) -> np.ndarray:
        """Batch embed — vectorized for ST, sequential for Ollama."""
        if self._st_model is not None:
        ...
        # Ollama has no native batch API — loop with cache
    ...
    def cosine_matrix(self, query_vec: np.ndarray, corpus_matrix: np.ndarray) -> np.ndarray:
        """Cosine sim: vectors already normalized → dot product = cosine."""
        return corpus_matrix @ query_vec   # shape: (N,)

    def top_k(self, scores: np.ndarray, k: int = 10) -> np.ndarray:
        """argpartition O(N) instead of argsort O(N log N)."""
        if len(scores) <= k:
